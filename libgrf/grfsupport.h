...
#define GRF_OffsetSort (&GRF_OffsetSort_Func)
...
GRFEXPORT char * GRFLINKAGE GRF_normalize_path(char *out, const char *in);
GRFEXPORT uint32_t GRFLINKAGE GRF_NameHash(const char *name);

typedef int (__cdecl * GrfSortCallback)(const void *, const void *);

GRFEXPORT void GRFLINKAGE grf_sort(Grf *grf, GrfSortCallback);
GRFEXPORT int __cdecl GRF_AlphaSort_Func(const GrfFile *g1, const GrfFile *g2);
GRFEXPORT int __cdecl GRF_OffsetSort_Func(const GrfFile *g1, const GrfFile *g2);

GRFEXPORT GrfFile * GRFLINKAGE grf_find(Grf *grf, const char *fname, uint32_t *index);

int GRF_list_from_array(Grf *grf, GrfError *error);
int GRF_array_from_list(Grf *grf, GrfError *error);

GRFEXPORT GrfError * GRFLINKAGE GRF_SetError(GrfError *err, GrfErrorType errtype, uint32_t line, const char *file, const char *func, void *extra);
GRFEXPORT const char * GRFLINKAGE grf_strerror(GrfError err);
